...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(100.0 / 255, 200.0 / 255, 100.0 / 255));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(255.0 / 255, 0.0 / 255, 200.0 / 255));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_K)) {
            cutoffAngle += deltaTime * SENSITIVITY;
            if (cutoffAngle > 360)
                cutoffAngle = 360;
        }
        ...
        rotation = glm::rotate(rotation, angleOY, glm::vec3(0, 1, 0));
        rotation = glm::rotate(rotation, angleOX, glm::vec3(1, 0, 0));
        lightDirection  = initialLightDirection;
        lightDirection  = glm::vec3(rotation * glm::vec4(lightDirection, 0));
    }
}
