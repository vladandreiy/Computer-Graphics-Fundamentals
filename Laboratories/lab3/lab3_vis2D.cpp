...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_W))
        logicSpace.y += deltaTime * SPEED_FACTOR;
    if (window->KeyHold(GLFW_KEY_S))
        logicSpace.y -= deltaTime * SPEED_FACTOR;
    if (window->KeyHold(GLFW_KEY_D))
        logicSpace.x += deltaTime * SPEED_FACTOR;
    if (window->KeyHold(GLFW_KEY_A))
        logicSpace.x -= deltaTime * SPEED_FACTOR;

    // TODO(student): Zoom in and zoom out logic window with Z and X
    if (window->KeyHold(GLFW_KEY_Z)) {
        // To zoom in/ zoom out from the center of the logic space, 
        // center x and y by getting half of the width and height
        logicSpace.x += logicSpace.width / 2;
        logicSpace.y += logicSpace.height / 2;
        logicSpace.width += deltaTime * ZOOM_FACTOR;
        logicSpace.height += deltaTime * ZOOM_FACTOR;
        logicSpace.x -= logicSpace.width / 2;
        logicSpace.y -= logicSpace.height / 2;
    }
