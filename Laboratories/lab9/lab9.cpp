...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        // glm::vec3 cameraPos = GetSceneCamera()->GetViewMatrix()[1];
        // modelMatrix = glm::mat4(gluLookAt(cameraPos.x, cameraPos.y, cameraPos.z, 0, 0, 0, 0, 1, 0));
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    bool mixTextures = (texture2 != NULL) ? true : false;
    glUniform1i(glGetUniformLocation(shader->program, "mixTextures"), mixTextures);

    // bool time = (meshes["sphere"] == mesh) ? true : false;
    float time = (meshes["sphere"] == mesh) ? Engine::GetElapsedTime() : -1;
