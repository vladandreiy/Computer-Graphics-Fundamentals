...
{
    glm::vec3 sphere = glm::vec3(0, 1, 1);
    {
    ...
    // Render ground
    {
        ...
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.4, 0.4, 0.4));
    }

    // Render the point light in the scene
    {
        // BONUS: Rotate light around sphere

        // Comment these lines to disable rotation
        lightPosition[0] -= sphere[0];
        lightPosition[1] -= sphere[1];
        float lightX = lightPosition[0] * cos(angularStep) - lightPosition[1] * sin(angularStep);
        float lightY = lightPosition[0] * sin(angularStep) + lightPosition[1] * cos(angularStep);
        lightPosition[0] = lightX + sphere[0];
        lightPosition[1] = lightY + sphere[1];
        // 
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform3fv(glGetUniformLocation(shader->program, "eye_position"),
                1,
                glm::value_ptr(eyePosition));

    // TODO(student): Set material property uniforms (shininess, kd, ks, object color)
    glUniform1i(glGetUniformLocation(shader->program, "material_shininess"),
                materialShininess);

    glUniform1f(glGetUniformLocation(shader->program, "material_kd"),
                materialKd);

    glUniform1f(glGetUniformLocation(shader->program, "material_ks"),
                materialKs);

    glUniform3fv(glGetUniformLocation(shader->program, "object_color"),
                1,
                glm::value_ptr(color));
